...
from collab_utils import read_impressions_tsv
...
class MatrixFactorizationRecommender:
    ...
    def __init__(self, n_factors=10, n_iterations=20, learning_rate=0.01, 
    ...
    def create_x(self, df):
        ...
        Returns:
            X (csr_matrix): Sparse user-item interaction matrix
            uid2index (dict): Mapping from user IDs to matrix indices
            nid2index (dict): Mapping from news IDs to matrix indices
            index2uid (dict): Mapping from matrix indices to user IDs
            index2nid (dict): Mapping from matrix indices to news IDs
        """
    ...
    def fit(self, df):
        ...
        X, self.uid2index, self.nid2index, self.index2uid, self.index2nid = self.create_x(df)
